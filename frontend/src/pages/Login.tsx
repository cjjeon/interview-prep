...
const Login = () => {
    ...
    const handleOAuthLogin = async (provider: Provider) => {
        login(undefined, undefined, provider).then(() => {
    ...
    const handleLogin = async () => {
        if (email === "") {
            ...
            login(email, password, undefined).then(() => {
                navigate(COMPANY_LIST_PAGE.path)
            })
        }
    }
    ...
    return (
        <div className={"flex justify-center items-center w-full h-screen"}>
            ...
            >
                <div
                    className={
                        "flex flex-col gap-5 border-0 md:border-r-2 px-10 pt-10 md:p-10"
                    }
                >
                    <div>
                    ...
                    <div>
                        ...
                        <div className={"pb-2"}>
                            <Link
                                className={"text-sm text-blue-300 underline my-2"}
                                to={FORGOT_PASSWORD_PAGE.path}
                            >
                                Forgot your Password?
                            </Link>
                        </div>
                        <SimpleButton label={"Login"} onClick={handleLogin} />
                    </div>
                    <div>
                        <Link
                            className={"text-right text-base text-blue-300 my-2"}
                            to={SIGNUP_PAGE.path}
                        >
                ...
                <div className={"flex flex-col gap-5 px-10 md:p-10 w-96"}>
                    ...
                    />
                    <SimpleButton
                        label={"Try Without Login"}
                        onClick={() => navigate(COMPANY_LIST_PAGE.path)}
                        color={"indigo"}
                    />
                </div>
            </div>
